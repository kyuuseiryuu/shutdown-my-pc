...
- **Single Executable** — Compile into one `.exe` with tray icon and server; set as Windows startup program
...
2. Create a shortcut to `out/ShutdownMyPC.exe`.
3. The server and tray icon will now start automatically on every boot.
...
├── tray/
│   ├── TrayApp.cs           # System tray wrapper (C#)
│   ├── tray-icon.ico        # Custom tray icon
│   └── ServerSize.cs        # Auto-generated by build.js
